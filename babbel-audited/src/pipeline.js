...
class Direction {
  ...
  async _process(utterance, capturedAt) {
    ...
    // Show the source text too when it differs - otherwise a bad translation
    // is indistinguishable from a bad transcription.
    const shown = translated === text ? `"${translated}"` : `"${text}" -> "${translated}"`;
    this.log.line(
      `[${this.name}] ${srcLang}->${this.targetLanguage}  ${shown}` +
