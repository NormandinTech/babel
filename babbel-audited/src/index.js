...
function preflight(cfg, log) {
  ...
  // whisper's -tr flag only produces English. Any other outgoing target with the
  // whisper backend throws on every single utterance, which reads as a mysterious
  // silent failure rather than a configuration mistake.
  if (cfg.outgoing.enabled &&
      cfg.outgoing.targetLanguage &&
      cfg.outgoing.targetLanguage !== 'en' &&
      cfg.translate.backend === 'whisper') {
    problems.push(
      `outgoing.targetLanguage is '${cfg.outgoing.targetLanguage}' but translate.backend is ` +
      `'whisper', which only outputs English. Set the target to 'en', or set the backend to ` +
      `'llama' and run llama-server on port 8080.`
    );
  }
...
async function main() {
  ...
  // Register installed voices FIRST. Language detection below checks which
  // voices exist, so doing it the other way round meant a user with, say,
  // German installed would still be told German was unavailable.
  ...
  if (!cfg.incoming.targetLanguage) {
    ...
    const file = cfg.tts.voices[detected];
    if (file && fs.existsSync(file)) {
      ...
      const known = voices.CATALOG[detected] || VOICE_CATALOG[detected];
      log.warn(
        known
          ? `system language is '${detected}' but that voice isn't installed - using English. ` +
            `Get it with:  node src\\index.js --add-voice ${detected}`
          : `no voice available for system language '${detected}' - using English.`
      );
    }
  }
